...
def get_db():
    import os as _os, sqlite3 as _sqlite3, time
    url = (_os.getenv("DATABASE_URL", "") or "").strip()

    # Normalize scheme and FORCE sslmode=require for managed PG
    if url.startswith("postgres://"):
        url = url.replace("postgres://", "postgresql://", 1)
    ...
    if url.startswith("postgresql://"):
        ...
        class QMarkCursor:
            ...
            def execute(self, query, vars=None):
                if vars is not None: query = query.replace("?", "%s")
        ...
        class PooledConn:
            ...
            def cursor(self):  return QMarkCursor(self._conn.cursor())
        ...
        # Get a connection and ensure schema (retry once on transient SSL drops)
        for attempt in (1, 2):
            try:
                ...
                try:
                    ...
                    pass
                if attempt == 1:
                    print("[DB] init failed, retrying once:", repr(e))
                    time.sleep(0.5)
                else:
                    raise
